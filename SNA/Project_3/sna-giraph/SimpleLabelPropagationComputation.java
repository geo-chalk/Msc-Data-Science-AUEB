...
public class SimpleLabelPropagationComputation
    ...
    public static final int MAX_SUPERSTEPS = 60;
    ...
    public void compute(
        ...
        // if superstep is not the first and less than the MAX_SUPERSTEPS proceed
            ...
            if (result.size() == 1) {
                if (vertex.getValue().get() < result.get(0)){
                ...
                LOG.info(result + " freq: " + max_freq + " Community:" + current_min + "\n");
                
                // if vertex value changes propagate
                if (!vertex.getValue().equals(new LongWritable(current_min))) {
                    LOG.info("Vertex_value: " + vertex.getValue() + " New:" + current_min + "\n\n");
